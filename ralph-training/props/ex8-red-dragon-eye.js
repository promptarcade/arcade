// Exercise 8: Red variant of full dragon eye at 192x128
// SAME structure as exercise 7 — all techniques carry over
// NEW palette: red/crimson scales, orange/yellow iris
// TRANSFERABLE: How to transpose a composition to a different colour temperature
// while preserving form and value structure.
// HUE PRE-COMPENSATION: engine shifts shadows +20°. Red base (H≈0°) shadows shift
// toward orange (H≈20°). Pre-compensate with cooler reds (H≈345-350°, more crimson/magenta)
// so shadows land in deep red, not orange-brown.

module.exports = {
  width: 192, height: 128, style: 'hd', entityType: 'prop',
  outlineMode: 'none',
  colors: {
    // RED SCALE PALETTE — pre-compensated for engine hue shift
    // Cool (shadow): H≈348° (slightly magenta-red) L≈24% — shadows shift +20° → H≈8° (deep warm red)
    cool: '#6a1828',
    // Warm (highlight): H≈355° L≈72% — highlights shift -15° → H≈340° (stays red/pink)
    warm: '#d47070',
    // IRIS PALETTE — warm orange/yellow for contrast against red scales
    // Deep: H≈30° (orange) L≈28% — shadows shift +20° → H≈50° (stays warm gold)
    irisDeep: '#7a4810',
    // Bright: H≈42° (gold/yellow) L≈72% — highlights shift -15° → H≈27° (warm amber)
    irisBright: '#e8b838',
  },
  ...
  drawPost(pc, pal) {
    ...
    // Scale tone: maps 0-15 across cool+warm groups (red palette)
    function scaleTone(i) {
      i = Math.max(0, Math.min(15, Math.round(i)));
      return i < 8 ? cg.startIdx + i : wg.startIdx + (i - 8);
    }

    // Iris tone: maps 0-15 across irisDeep+irisBright groups (orange/gold palette)
    function irisTone(i) {
      i = Math.max(0, Math.min(15, Math.round(i)));
    ...
    // Fill background with dark noise (deep red/maroon)
    for (let y = 0; y < H; y++)
      for (let x = 0; x < W; x++) {
    ...
    // ============================================================
    // PASS 0: SURROUNDING SCALES (Voronoi, zone-based, RED palette)
    ...
    // Render scales with zone-based rendering (RED palette)
    ...
    // ============================================================
    // PASS 1: EYEBALL / IRIS (orange/gold palette, fills ENTIRE socket)
    ...
    // ============================================================
    // PASS 2: EYELID RIDGE (red palette, cylinder normals)
