// Glyph Forge — Rune Fragment: Cross
// 48x48 glowing cross/plus shape on dark stone cell
module.exports = {
  ...
  drawPost(pc, pal) {
    ...
    // Cross — vertical and horizontal through centre
    const strokeW = 2.8;
    const armLen = 14;
    const segments = [
      { x0: cx, y0: cy - armLen, x1: cx, y1: cy + armLen },   // vertical
      { x0: cx - armLen, y0: cy, x1: cx + armLen, y1: cy },   // horizontal
    ];
    ...
    for (let y = 2; y < H - 2; y++) {
      for (let x = 2; x < W - 2; x++) {
        ...
        if (minDist < strokeW) {
          ...
          const v = f * f;
          // Bright centre intersection
          const centerDist = Math.sqrt((x - cx) * (x - cx) + (y - cy) * (y - cy));
          const centerBoost = Math.max(0, 1 - centerDist / 6) * 0.35;
          if (v + centerBoost > 0.5) {
            pc.setPixel(x, y, tone(cg, Math.min(1, 0.4 + (v + centerBoost) * 0.5)));
          } else {
