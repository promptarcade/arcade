// Level 4.2: Chef + 3 Customers — coloured with temperature shifts
// Each character has distinct colour scheme, all with warm lit / cool shadow
// Chef: white toque+apron, blue shirt | Customer1: red shirt, blonde
// Customer2 (kid): green shirt, dark hair | Customer3 (elderly): purple cardigan, grey hair
// Layout: 4 characters (136 × 50)
module.exports = {
  width: 136,
  height: 50,
  style: 'chibi',
  ...
  drawPost(pc, pal) {
    ...
    function dt(wg, cg, v, thr) {
      thr = thr || 0.38;
      return v > thr ? tone(wg, v) : tone(cg, Math.max(0.04, v * 1.2));
    }
    ...
    // ========================================
    // DRAW CHARACTER — reusable function
    // ========================================
    function drawCharacter(cx, opts) {
      ...
      for (let y = armTop; y < armTop + armH; y++) {
        for (let dx = 0; dx < 3; dx++) {
          ...
          if (lax >= 0) px(lax, y, shirtMat, cylinderV(lax, tL - 1.5, 1.5) * 0.7);
          if (rax < TW) px(rax, y, shirtMat, cylinderV(rax, tL + torsoW + 1.5, 1.5));
        }
      }
      // Hands
      ...
      return { tL, torsoW, torsoTop, torsoH, neckTop, headCy, headRx, headRy, armTop, armH, legY, legH };
    }
    ...
    // ========================================
    {
      ...
      const parts = drawCharacter(cx, { hair: chHr, shirt: chSh, headCy: 14 });
    ...
    // ========================================
    {
      ...
      drawCharacter(cx, { hair: c1Hr, shirt: c1Sh });

      // Long hair (blonde) flowing past shoulders
      for (let y = 8; y <= 30; y++) {
        ...
        if (w > 0) {
          for (let dx = 0; dx < w; dx++) {
            ...
            if (lhx >= 0) px(lhx, y, c1Hr, 0.2);
            if (rhx < TW) px(rhx, y, c1Hr, 0.45);
          }
        }
      }
    }
    ...
    // ========================================
    // 3. CUSTOMER 2 — kid, dark hair, green shirt
    // ========================================
    {
      const cx = 85;
      ...
      drawCharacter(cx, { hair: c2Hr, shirt: c2Sh, scale: 0.8, headCy: 16 });
    ...
    // ========================================
    // 4. CUSTOMER 3 — elderly, grey hair, purple cardigan
    // ========================================
    {
      const cx = 119;
      ...
      drawCharacter(cx, { hair: c3Hr, shirt: c3Sh });
